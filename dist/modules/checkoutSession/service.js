...
const createCheckoutSessionService = async (body) => {
    ...
    if (existingSession) {
        const updatedCheckoutSession = await model_1.default.findOneAndUpdate({ guestId: validatedBody.guestId }, {
            $set: {
                items: itemsToSave,
    ...
    const checkoutSession = await model_1.default.create([
        {
            guestId: validatedBody.guestId,
            items: itemsToSave,
            expiresAt,
    ...
    for (const item of validatedBody.items) {
        ...
        product.reservedStock = product.reservedStock.filter((r) => new Date(r.expiresAt) > now);
        const userReservation = product.reservedStock.find((r) => r.guestId === validatedBody.guestId);
    ...
    for (const item of validatedBody.items) {
        ...
        const existingReservation = product.reservedStock.find((r) => r.guestId === validatedBody.guestId);
